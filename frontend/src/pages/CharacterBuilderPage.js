...
import { Plus, Users, Wand2, Trash2, Pencil, X, Sparkles } from 'lucide-react';
...
export const CharacterBuilderPage = () => {
  ...
  const resetForm = () => { setForm({ ...emptyChar }); setEditId(null); setShowForm(false); };
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          {showForm && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm">
              <motion.div
                ...
                data-testid="character-form-modal"
              >
                ...
                <form onSubmit={handleSave} className="space-y-5">
                  <div className="space-y-2">
                    ...
                    <Input data-testid="char-name-input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Character name" className="h-12 rounded-xl border-2 border-slate-200 focus:border-[#6366F1]" required />
                  </div>

                  <div className="space-y-2">
                    <Label className="text-sm font-medium">Role</Label>
                    <Select value={form.role} onValueChange={(v) => setForm({ ...form, role: v })}>
                      <SelectTrigger className="h-12 rounded-xl border-2 border-slate-200" data-testid="char-role-select">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea data-testid="char-description-input" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Describe your character..." className="rounded-xl border-2 border-slate-200 focus:border-[#6366F1] min-h-[80px]" />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={form.speaking_style} onValueChange={(v) => setForm({ ...form, speaking_style: v })}>
                      <SelectTrigger className="h-12 rounded-xl border-2 border-slate-200" data-testid="char-speaking-style-select">
                  ...
                  <Button type="submit" disabled={saving} data-testid="save-character-btn" className="w-full h-12 rounded-full bg-[#6366F1] hover:bg-[#4F46E5] text-white font-accent shadow-lg shadow-[#6366F1]/25 hover:scale-[1.02] active:scale-95 transition-all">
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-panel rounded-3xl p-16 text-center">
            ...
            <Button onClick={() => setShowForm(true)} data-testid="empty-add-character-btn" className="rounded-full bg-[#6366F1] hover:bg-[#4F46E5] text-white font-accent px-8 shadow-lg shadow-[#6366F1]/25">
            ...
            {characters.map((char, i) => (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-white font-heading font-bold text-lg" style={{ backgroundColor: roleColors[char.role] || '#6366F1' }}>
                    {char.name[0]?.toUpperCase()}
                  </div>
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => startEdit(char)} data-testid={`edit-char-${char.id}`} className="p-2 text-[#94A3B8] hover:text-[#6366F1] rounded-lg hover:bg-[#EEF2FF]">
                      <Pencil className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleDelete(char.id)} data-testid={`delete-char-${char.id}`} className="p-2 text-[#94A3B8] hover:text-red-500 rounded-lg hover:bg-red-50">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                </div>
                <h3 className="font-heading font-bold text-lg text-[#1E293B] mb-1">{char.name}</h3>
                <Badge className="rounded-full text-xs font-medium mb-3 capitalize" style={{ backgroundColor: (roleColors[char.role] || '#6366F1') + '15', color: roleColors[char.role] || '#6366F1' }}>
                  {char.role}
                </Badge>
